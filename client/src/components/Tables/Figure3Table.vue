  ...
  <div class="row">
    <div class="col-md-12">
      <el-table :data="summary">
        <el-table-column label="Metric (kg CO2eq avoided/kg Product)">
          ...
          <el-table-column prop="avoidedEmission" label="Avoided Emission" :formatter="formatter"></el-table-column>
        </el-table-column>
      </el-table>
    </div>
  </div>
  ...
  methods: {
    update(pathway) {
      ...
      if (!this.ready.includes(false)) {
        ...
        this.$store.dispatch("summary/updateFigure3", this.temp).then(() => {
          Event.$emit("ready");
        });
